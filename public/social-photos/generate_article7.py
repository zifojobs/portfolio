"""Generate les 7 slides Instagram + 7 slides WhatsApp pour l'article 7 (Site bilingue diaspora senegalaise)."""
import asyncio
import os
...
# Halos de couleur (le "style coloré" valide par Saibo)
...
SLIDES = [
    {
        "type": "hero",
        "number": "07",
        "title": "Un site qui parle aux deux mondes",
        "subtitle_main": "Site web bilingue pour la diaspora sénégalaise",
        "subtitle_note": "Un guide par Saïbo Danfakha",
    },
    {
        "type": "error",
        "title": "CLÉ 1",
        "error_name": "VOS 2 AUDIENCES",
        "points": [
            "Au pays : WhatsApp, FCFA, proximité, confiance",
            "À l'international : pro, références, devises",
            "Nommez-les AVANT de choisir la langue",
        ],
    },
    {
        "type": "error",
        "title": "CLÉ 2",
        "error_name": "PAS TOUT TRADUIRE",
        "points": [
            "Un bon site bilingue n'est pas une copie carbone",
            "Même charte, deux conversations différentes",
            "FR : Wave / Orange Money · EN : Stripe / RDV",
        ],
    },
    {
        "type": "error",
        "title": "CLÉ 3",
        "error_name": "QUELLE 2e LANGUE ?",
        "points": [
            "FR + EN → Amérique du Nord, partenaires",
            "FR + Wolof → proximité, slogans, messages clés",
            "FR seul bien ciblé > 2 langues bâclées",
        ],
    },
    {
        "type": "error",
        "title": "CLÉ 4",
        "error_name": "PIÈGES TECHNIQUES",
        "points": [
            "hreflang faux = vos 2 versions chutent",
            "1 URL par langue (/fr/ et /en/)",
            "Jamais de langue forcée selon le pays",
        ],
    },
    {
        "type": "error",
        "title": "CLÉ 5",
        "error_name": "LES CODES DES 2 MONDES",
        "points": [
            "Paiement : mobile money vs carte / PayPal",
            "Contact : WhatsApp vs e-mail / Calendly",
            "Dates, devises, références : « pensé pour moi »",
        ],
    },
    {
        "type": "error",
        ...
        "points": [
            "Nommez vos 2 audiences précisément",
            "Vraiment 2 langues ? sinon 1 langue, 2 messages",
            "Le bilingue se décide AU DÉBUT du projet",
        ],
    },
]
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for idx, slide in enumerate(SLIDES, start=1):
            ...
            ig_path = os.path.join(BASE_DIR, f"article7-instagram-{idx:02d}.html")
            ig_png = os.path.join(BASE_DIR, f"Instagram 07-{idx:02d}.png")
            ...
            await page.close()
            print(f"[OK] Instagram 07-{idx:02d}.png")

            # WhatsApp (1080x1920)
            wa_html = build_whatsapp_slide(slide, idx, len(SLIDES))
            wa_path = os.path.join(BASE_DIR, f"article7-whatsapp-{idx:02d}.html")
            wa_png = os.path.join(BASE_DIR, f"Whatsapp 07-{idx:02d}.png")
            ...
            await page.close()
            print(f"[OK] Whatsapp 07-{idx:02d}.png")

        await browser.close()
        print("\n[SUCCESS] Article 7 - 14 slides generees (7 Instagram + 7 WhatsApp)")
